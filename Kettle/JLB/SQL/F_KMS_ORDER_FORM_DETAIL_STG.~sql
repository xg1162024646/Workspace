...
BEGIN
  ...
  FROM ALL_TABLES
  WHERE TABLE_NAME = 'F_KMS_ORDER_FORM_DETAIL_STG';
  
  IF V_NUM = 1 THEN
    EXECUTE IMMEDIATE 'DROP TABLE F_KMS_ORDER_FORM_DETAIL_STG';
  END IF;
END;
/
CREATE TABLE F_KMS_ORDER_FORM_DETAIL_STG
(
  ID NUMBER GENERATED BY DEFAULT AS IDENTITY (START WITH 10000 INCREMENT BY 1) PRIMARY KEY
, INVOICES_ID NUMBER
